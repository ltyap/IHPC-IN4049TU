...
double time_Solve_start, time_Solve, time_Solve_total = 0.0;
double time_exch_start, time_exch, time_exch_total = 0.0;
double time_comm_start, time_comm, time_comm_total = 0.0;
double time_comp_start, time_comp, time_comp_total = 0.0;
double time_idle_total = 0.0;
...
void print_timer()
{
    ...
    if (proc_rank == 0)
    {
        if ((timer = fopen("timer.dat", "a")) == NULL)
            Debug("print_timer : fopen failed", 1);
    
        if (timer_on)
        {
            stop_timer();
            time_idle_total = wtime - time_comp_total - time_comm_total - time_exch_total;
            fprintf(timer, "%i \t %.5f \t %.5f \t %.5f \t %.5f \t %.5f\n",
                proc_rank, time_comp_total, time_comm_total, time_exch_total, time_idle_total, wtime);
            resume_timer();
        }
        else
        {
            time_idle_total = wtime - time_comp_total - time_comm_total - time_exch_total;
            fprintf(timer, "%i \t %.5f \t %.5f \t %.5f \t %.5f \t %.5f \t %.5f\n",   
                proc_rank, time_comp_total, time_comm_total, time_exch_total, time_idle_total, time_Solve, wtime);
        }
    

        for (int p = 1; p < P; p++)
        {
            MPI_Recv(&time_comp_total, 1, MPI_DOUBLE, p, 0, grid_comm, &status);
            MPI_Recv(&time_comm_total, 1, MPI_DOUBLE, p, 0, grid_comm, &status);
            MPI_Recv(&time_exch_total, 1, MPI_DOUBLE, p, 0, grid_comm, &status);
            MPI_Recv(&time_idle_total, 1, MPI_DOUBLE, p, 0, grid_comm, &status);
            MPI_Recv(&time_Solve, 1, MPI_DOUBLE, p, 0, grid_comm, &status);
            MPI_Recv(&wtime, 1, MPI_DOUBLE, p, 0, grid_comm, &status);
            fprintf(timer, "%i \t %.5f \t %.5f \t %.5f \t %.5f \t %.5f \t %.5f\n",   
                p, time_comp_total, time_comm_total, time_exch_total, time_idle_total, time_Solve, wtime);
    ...
    {
        time_idle_total = wtime - time_comp_total - time_comm_total - time_exch_total;
        MPI_Send(&time_comp_total, 1, MPI_DOUBLE, 0, 0, grid_comm);
        MPI_Send(&time_comm_total, 1, MPI_DOUBLE, 0, 0, grid_comm);
        MPI_Send(&time_exch_total, 1, MPI_DOUBLE, 0, 0, grid_comm);
        MPI_Send(&time_idle_total, 1, MPI_DOUBLE, 0, 0, grid_comm);
        MPI_Send(&time_Solve, 1, MPI_DOUBLE, 0, 0, grid_comm);
        MPI_Send(&wtime, 1, MPI_DOUBLE, 0, 0, grid_comm);
    }
}
...
void Setup_Proc_Grid()
{
 ...
 /* double time_idle_start, time_idle_end, time_idle_total =0.0;
  double time_comm_start, time_comm_end, time_comm_total = 0.0;
  double time_exch_start, time_exch_end, time_exch_total = 0.0;
  double time_comp_start, time_comp_end, time_comp_total = 0.0; 
  double time_Setup_start, time_Setup_end, time_Setup;*/

 // time_Setup_start = MPI_Wtime();
 ...
 // time_comm_total += time_comm_end - time_comm_start;
 ...
 // time_comp_end = MPI_Wtime();
 // time_comp_total = time_comp_end - time_comp_start;
 ...
 // time_comp_end = MPI_Wtime();
 // time_comp_total = time_comp_total + (time_comp_end - time_comp_start);
...
//  time_comm_end = MPI_Wtime();
 ...
 // time_idle_total = time_Setup - time_comp_total - time_comm_total - time_exch_total;
/*
  printf("Time spent by process %d for calculation: %10.5lf\n", proc_rank, time_comp_total);
  printf("Time spent by process %d for communication: %10.5lf\n", proc_rank, time_comm_total);
  printf("Time spent by process %d for exchange: %10.5lf\n", proc_rank, time_exch_total);
  printf("Time spent by process %d for idling: %10.5lf\n", proc_rank, time_idle_total);
  printf("Time spent by process %d in Setup_Proc_Grid : %10.5lf\n", proc_rank, time_Setup);*/
...
void Solve() {
  ...
  time_Solve_start = MPI_Wtime();

  time_exch_start = MPI_Wtime();
  Exchange_Borders(phi);
  time_exch = MPI_Wtime() - time_exch_start;
  time_exch_total += time_exch;
  ...
  time_comp = MPI_Wtime() - time_comp_start;
  time_comp_total += time_comp;
  ...
  time_Solve = MPI_Wtime() - time_Solve_start;
  time_Solve_total += time_Solve;
  free(q);
  free(p);
  free(r);
  if (proc_rank == 0)
  {
    ...
    // time_idle_total = time_Solve_total - time_comp_total - time_comm_total - time_exch_total;
   //   printf("Time spent by process %d for calculation: %10.5lf\n", proc_rank, time_comp_total);
   //   printf("Time spent by process %d for communication: %10.5lf\n", proc_rank, time_comm_total);
   //   printf("Time spent by process %d for exchange: %10.5lf\n", proc_rank, time_exch_total);
 // printf("Time spent by process %d for idling: %10.5lf\n", proc_rank, time_idle_total);
  //    printf("Time spent by process %d in Solve : %10.5lf\n", proc_rank, time_Solve);
  }
}
